Shader "Hidden/VFX/System 4/Quad Output" {
	Properties {
	}
	SubShader {
			...
			Program "vp" {
				SubProgram "d3d11 " {
					...
					void main()
					{
					    ...
					    u_xlatu3 = (-floatBitsToUint(u_xlat3.x)) + floatBitsToUint(nbMax);
					    ...
					    u_xlat3.x = uintBitsToFloat(attributeBuffer_buf[(u_xlati3.x >> 2) + 0]);
					    u_xlati6 = u_xlati3.y + 361728;
					    ...
					    u_xlat1.x = u_xlat3.x * Size_a.x + Size_a.y;
					    ...
					    u_xlat3.x = u_xlat3.x * Alpha_b.x + Alpha_b.y;
					    u_xlatu7 = floatBitsToUint(Alpha_b.w) >> 2u;
					    switch(int(u_xlatu7)){
					        case 1:
					            u_xlat7 = min(u_xlat3.x, 1.0);
					            u_xlat7 = fract(u_xlat7);
					            u_xlat3.x = u_xlat7 * 0.9921875 + 0.00390625;
					            break;
					        case 2:
					    ...
					    u_xlati0 = int(floatBitsToUint(Alpha_b.w) & 3u);
					    u_xlat3.z = Alpha_b.z;
					    u_xlat1 = textureLod(bakedTexture, u_xlat3.xz, 0.0);
					    vs_COLOR0.w = dot(u_xlat1, ImmCB_0_0_0[u_xlati0]);
